...
def get_dir(args):
    saved_model_folder = os.path.join(args.save_path, args.name, 'models')
    saved_image_folder = os.path.join(args.save_path, args.name, 'images')
    ...
    for f in os.listdir('./'):
        if '.py' in f:
            shutil.copy(f, os.path.join(args.save_path, args.name, f))
